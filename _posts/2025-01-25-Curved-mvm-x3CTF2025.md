...
tags: [Curvas Elípticas, Writeups, Media]
pin: false
math: true
mermaid: true
image:
  path: https://github.com/k3sero/Blog_Content/blob/main/Competiciones_Internacionales_Writeups/2025/x3CTF2025/Cripto/curved-mvm/img/logo.png?raw=true
  lqip: 
  alt: 
comments: true
---
...
4. La función $sign_msg$ firma un mensaje utilizando el esquema de firma `ECDSA` (Elliptic Curve Digital Signature Algorithm).
...
Esto es posible porque el valor de $k$ se genera con un tamaño de solo 18 bits K_SIZE = 18, lo que significa que hay solo $$2^18 = 262,144$$ valores posibles para $k$. Esto es un número manejable para un ataque de fuerza bruta.
...
2. Calculamos el hash $z$ del mensaje $SAMPLE_MSG$ usando `SHA-1`.

3. Realizamos fuerza bruta de $k$ iterando sobre todos los posibles valores de $k$ (desde 0 hasta 2^18 - 1).

4. Para cada valor de k, se calcula $$R = k * G$$ y obtenemos un $$r_candidate = ZZ(R.x()) % n$$

5. Si $r_candidate$ coincide con el valor $r$ de la firma, entonces hemos encontrado el valor correcto de $k$.

6. Una vez que tenemos $k$, puedes resolver la ecuación de la firma para obtener la clave secreta `SECRET_KEY`:
     $$s = k^{-1} (z + r \cdot SECRET_KEY) \mod n$$
     Despejando `SECRET_KEY`:
     $$
     SECRETKEY = (s \cdot k - z) \cdot r^{-1} \mod n
     $$

7. Por último, firmamos un mensaje en local para posteriormente enviarlo a el servidor usando la funcion mvm, ya que la firmas realizadas de forma local son válidas.
...
![Obtencion](https://github.com/k3sero/Blog_Content/blob/main/Competiciones_Internacionales_Writeups/2025/x3CTF2025/Cripto/curved-mvm/img/obtain.png?raw=true)

Acto seguido introducimos las firmas en los parametros r y s correspondientes y ejecutamos el script

![Ejecución](https://github.com/k3sero/Blog_Content/blob/main/Competiciones_Internacionales_Writeups/2025/x3CTF2025/Cripto/curved-mvm/img/execution.png?raw=true)

Por último, introducimos las claves válidas generadas en local para obtener la flag.

![flag](https://github.com/k3sero/Blog_Content/blob/main/Competiciones_Internacionales_Writeups/2025/x3CTF2025/Cripto/curved-mvm/img/flag.png?raw=true)
